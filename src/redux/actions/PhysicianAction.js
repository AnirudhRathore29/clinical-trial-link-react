...
export const PhysicianClinicAppTrialListAction = (url, data) => async (dispatch) => {
    dispatch(Request());
    axios
...
export const PhysicianManageAllPatientListAction = (data) => async (dispatch) => {
    ...
    axios
        ...
        .then(response => {
            dispatch({ type: PHYSICIAN_MANGE_PATIENT_LIST_SUCCESS, payload: response });
        })
        .catch(error => {
            dispatch({ type: PHYSICIAN_MANGE_PATIENT_LIST_ERROR, payload: error.response.data });
            HandleError(error.response.data)
        });
}
